...
// ===== 5. 유리병 편지 컴포넌트 =====
function FloatingBottle({ id }) {
  const bottleRef = useRef();
  const startX = -6; // 왼쪽 화면 밖
  const endX = Math.random() * 5 + 2; // 중앙 근처
  const startY = Math.random() * 0.5 + 0.2; // 화면 아래
  const startZ = Math.random() * -2; // 카메라 바로 앞

  const [position, setPosition] = useState(new THREE.Vector3(startX, startY, startZ));

  useFrame((state, delta) => {
    if (bottleRef.current) {
      position.x += delta * 0.5;
      if (position.x > endX) position.x = endX;
      position.y = startY + Math.sin(state.clock.elapsedTime + position.x) * 0.05;
...
function FullOceanScene() {
  ...
  const [particleCode, setParticleCode] = useState('PARTICLE_RAINY');
  ...
  return (
    <div style={{ width: '100vw', height: '100vh', overflow: 'hidden' }}>
      <Canvas camera={{ position: [30, 10, 20], fov: 75 }}>  
        {/* 조명 */}
