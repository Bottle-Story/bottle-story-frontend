...
export default function DawnMoonCloudy() {
  ...
  return (
    <>
      ...
      {/* 구름 레이어 */}
      <Clouds opacity={0.3} speed={0.1} width={200} depth={200} segments={30} />
      ...
      <mesh ref={glowRef} position={[0, 25, -122]} renderOrder={0}>
        ...
        <shaderMaterial
          ...
          fragmentShader={`varying vec2 vUv; void main(){ float dist=distance(vUv, vec2(0.5)); float alpha=0.015*(1.0-smoothstep(0.0,1.0,dist)); gl_FragColor=vec4(0.8,0.8,1.0,alpha);}`}
          transparent
          depthWrite={false}
      ...
      <ambientLight color={0x555577} intensity={0.06} />
    </>
  );
}
